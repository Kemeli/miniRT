...
{
	t_sphere s = create_sphere();
	t_intersect	*xs_list = NULL;
	t_intersect *i1 = create_intersection(1, (void *)&s);
	xs_list = add_intersection_to_list(xs_list, i1);
	t_intersect *i2 = create_intersection(2, (void *)&s);
	xs_list = add_intersection_to_list(xs_list, i2);
	int count = 0;
	t_intersect *aux;

	mu_assert_double_eq(1, xs_list->t);
	mu_assert_double_eq(2, xs_list->next->t);
