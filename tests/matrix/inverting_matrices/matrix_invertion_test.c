...
MU_TEST(test_if_a_matrix_is_invertible)
{
	...
	copy_size = 4;
	double **matrix_A = copy_matrix(matrix_values);
	double result = determinant(matrix_A);
	double expected = -2120;
...
MU_TEST(test_if_a_inverted_matrix_is_equal_to_a_received_matrix)
{
	...
	copy_size = 4;
	double **matrix_A = copy_matrix(matrix_A_values);
	double **matrix_inverse = inverse(matrix_A);
	...
	double matrix_expected_values[4][4] = {
		{0.21805, 0.45113, 0.24060, -0.04511},
		{-0.80827, -1.45677, -0.44361, 0.52068},
		{-0.07895, -0.22368, -0.05263, 0.19737},
		{-0.52256, -0.81391, -0.30075, 0.30639}
	};
	double **matrix_expected = copy_matrix(matrix_expected_values);
...
MU_TEST(test_if_inverse_function_returns_correct_matrices_1)
{
	...
	copy_size = 4;
	double	**matrix_A = copy_matrix(matrix_values);
	double	**expected_matrix = copy_matrix(expected_values);
	double	**inverse_A = inverse(matrix_A);
...
MU_TEST(test_if_multiplying_by_inverse_gets_back_to_original_matrix)
{
	...
	copy_size = 4;
	double	**matrix_A = copy_matrix(matrix_A_values);
	double	**matrix_B = copy_matrix(matrix_B_values);
	double	**matrix_C = multiply_matrix(matrix_A, matrix_B);
	double	**inverse_B = inverse(matrix_B);
	double	**result = multiply_matrix(matrix_C, inverse_B);
